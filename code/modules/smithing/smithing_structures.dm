...
/obj/structure/medieval/oven/forge/cook()
	if(istype(itemInside, /obj/item/stack/ingot))
		...
		if(istype(I, /obj/item/stack/ingot/iron) && I.heated)
			var/obj/item/stack/ingot/steel/S = new
			S.amount = round(I.amount * 0.8)
			S.forceMove(loc)
			S.heat()
			qdel(I)
		else
...
/obj/structure/medieval/oven/crucible/cook()
	itemInside.fire_act() //This won't go poorly at all
	if(itemInside) //Some things are replaced and "ejected" by fire_act(), others aren't. This deals with the latter (hopefully).
